...
object WaitingForFileVerification {
  implicit val formatter: Format[WaitingForFileVerification] = Json.format[WaitingForFileVerification]
}
...
trait FileUploadService {
  ...
  def initiateFileUpload(
    ...
    state match {
      ...
      case Some(WaitingForFileVerification(
      reference,
      uploadRequest,
      currentFileUpload,
      fileUploads
  ...
  final def waitForFileVerification(state: FileUploadState): Future[FileUploadState] = state match {
    ...
    fileUploads
      ...
      commonFileUploadStatusHandler(
        ...
        reference,
        uploadRequest,
        UploadFile(reference, uploadRequest, fileUploads)
      )
        .apply(currentUpload)
  ...
  final def upscanCallbackArrived(notification: UpscanNotification)(state: FileUploadState) = {
    ...
    state match {
      ...
      case current@UploadFile(reference, uploadRequest, fileUploads, errorOpt) =>
        val updatedFileUploads = updateFileUploads(fileUploads)
        val currentUpload = updatedFileUploads.files.find(_.reference == reference)
        commonFileUploadStatusHandler(
          updatedFileUploads,
  ...
  private def commonFileUploadStatusHandler(
    ...
    case Some(postedFile: FileUpload.Posted) =>
      Future.successful(WaitingForFileVerification(
        reference,
        uploadRequest,
        postedFile,
        fileUploads
