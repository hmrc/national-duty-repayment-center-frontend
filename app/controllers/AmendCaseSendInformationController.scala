...
import models.{AmendCaseResponseType, Mode, NormalMode, SessionState, UpscanNotification, UserAnswers}
  ...
  final def markFileUploadAsRejected(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async { implicit request =>
    UpscanUploadErrorForm.bindFromRequest().fold(
      ...
      s3Error =>
        sessionRepository.getFileUploadState(request.internalId).flatMap { ss =>
          ss.state match {
            case Some(s) => applyTransition(fileUploadWasRejected(s3Error)(_), s, ss).map(_ => Redirect(routes.AmendCaseSendInformationController.showFileUpload(mode)))
            case None => Future.successful(missingFileUploadState)
          }
        }
    )
  ...
  def backLink(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async { implicit request =>
    if (mode == NormalMode)
      ...
      for {
        ...
        res <- sessionRepository.updateSession(FileUploaded(fs.get.fileUploads.copy(files = filesNotInStateInitiated(fs.get.fileUploads.files))), ss.userAnswers)
        if res
