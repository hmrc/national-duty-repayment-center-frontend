            ...
            case Some(s) =>
              if (value)
                submitedUploadAnotherFileChoice(upscanRequest(request.internalId))(upscanInitiateConnector.initiate(_))(s).flatMap {
                  newState =>
                    newState match {
                      case s@UploadFile(_, _, _, _) =>
                        updateSession(newState, ss.userAnswers).map { _ => Redirect(routes.FileUploadController.showFileUploaded())}
                      case s => Future.successful(renderState(s))
                    }
                }
