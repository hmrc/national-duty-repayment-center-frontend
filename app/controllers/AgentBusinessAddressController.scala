...
import pages.{AgentBusinessAddressPage, ImporterPostcodePage}
...
class AgentBusinessAddressController @Inject()(
        ...
        selectionForm.bindFromRequest().fold(
          ...
          js =>
            form.bind(js).fold(
              ...
              address =>
                for {
                  updatedAnswers <- Future.fromTry(request.userAnswers.set(AgentBusinessAddressPage, address))
                  _              <- sessionRepository.set(updatedAnswers)
                } yield Redirect(routes.PhoneNumberController.onPageLoad(mode))
            )
        )
      }.getOrElse(Future.successful(Redirect(routes.AgentBusinessAddressController.enteredAddressPageLoad())))
  }

  def enteredAddressPageLoad(mode: Mode): Action[AnyContent] = (identify  andThen getData andThen requireData) {
    implicit request =>

      val preparedForm = request.userAnswers.get(AgentBusinessAddressPage) match {
        case None        => form
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(AgentBusinessAddressPage, value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(AgentBusinessAddressPage, mode, updatedAnswers))
      )
  }
