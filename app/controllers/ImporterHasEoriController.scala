...
class ImporterHasEoriController @Inject()(
  ...
  private def getBackLink(mode: Mode, userAnswers: UserAnswers): Call = {
    ...
    userAnswers.get(ClaimantTypePage) match {
      case Some(ClaimantType.Importer) => routes.FileUploadController.onPageLoad
      case _ => {
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(ImporterHasEoriPage, value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(ImporterHasEoriPage, mode, updatedAnswers))
      )
  }
}
