...
class FileUploadServiceSpec extends SpecBase with MustMatchers with ScalaCheckPropertyChecks with OptionValues {
  ...
  "at state FileUpload" should {
    ...
    "change state to WaitingForFileVerification when waitForFileVerification and not verified yet" in {
      ...
      whenReady(service.waitForFileVerification(currentState)) {
        newState => assert(newState == expectedState)
      }

    }
    "change state to FileUploaded when waitForFileVerification and accepted already" in {
    ...
    "change state to UploadFile when waitForFileVerification and file upload already rejected" in {
      val currentState =
        UploadFile(
          "foo-bar-ref-4",
          UploadRequest(
  ...
  "at state WaitingForFileVerification" should {
    "stay when waitForFileVerification and not verified yet" in {
      ...
      whenReady(service.waitForFileVerification(state)) {
        newState => assert(newState == state)
      }
    }
    ...
    "go to FileUploaded when waitForFileVerification and file already accepted" in {
          val currentState = WaitingForFileVerification(
          "foo-bar-ref-1",
          UploadRequest(
            href = "https://s3.bucket",
            fields = Map(
