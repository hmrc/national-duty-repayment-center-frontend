...
class AmendCaseResponseTypeSpec extends WordSpec with MustMatchers with ScalaCheckPropertyChecks with OptionValues {

  "AmendCaseResponseType" must {

    "deserialise valid values" in {

      val gen = Gen.oneOf(AmendCaseResponseType.values)
    ...
    "fail to deserialise invalid values" in {
      ...
      val gen = arbitrary[String] suchThat (!AmendCaseResponseType.values.map(_.toString).contains(_))
