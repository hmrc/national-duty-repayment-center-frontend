...
class EntryDetailsSpec extends AnyWordSpec with Matchers {
  "EntryDetails" must {
    "de-serialise local date" in {
      val json = JsString("20200805")
      EntryDetails.dateFormat.reads(json) mustBe JsSuccess(LocalDate.parse("2020-08-05"))
    }

    "return error on failing to read as LocalDate with date format as 'yyyyMMdd'" in {
      val json = JsString("20-02-2023")
      EntryDetails.dateFormat.reads(json).isError mustBe true
    }
