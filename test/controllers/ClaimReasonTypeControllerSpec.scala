...
import models.{ClaimReasonType, NormalMode, UserAnswers}
...
class ClaimReasonTypeControllerSpec extends SpecBase with MockitoSugar {
  ...
  "ClaimReasonType Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(ClaimReasonTypePage, ClaimReasonType.values.head).success.value
      ...
      contentAsString(result) mustEqual
        view(form.fill(ClaimReasonType.values.head), NormalMode)(request, messages).toString
