...
class ProofOfAuthorityUpscanCallbackControllerSpec extends SpecBase with MockitoSugar with BeforeAndAfterEach {
  ...
  "POST /callback-from-upscan/upload-proof-of-authority/:id" should {
    ...
    "return NO_CONTENT when fileUploadState is of type UploadFile" in {
      ...
      running(application) {
        ...
        val request =
          FakeRequest(POST, routes.ProofOfAuthorityUpscanCallbackController.callbackFromUpscan("id").url).withJsonBody(
            json
          )
