...
class AmendCaseUpscanCallbackControllerSpec extends SpecBase with MockitoSugar with BeforeAndAfterEach {
  ...
  "POST /callback-from-upscan/ndrc/:id" should {
    ...
    "return NO_CONTENT when fileUploadState is of type UploadFile" in {
      ...
      running(application) {
        ...
        val request =
          buildRequest(POST, routes.AmendCaseUpscanCallbackController.callbackFromUpscan("id").url).withJsonBody(json)
