...
import models.{NormalMode, RepaymentType, UserAnswers}
...
class RepaymentTypeControllerSpec extends SpecBase with MockitoSugar {
  ...
  "RepaymentType Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(RepaymentTypePage, RepaymentType.values.head).success.value
      ...
      contentAsString(result) mustEqual
        view(form.fill(RepaymentType.values.head), NormalMode, backLink)(fakeRequest, messages).toString
