...
class VATDueToHMRCControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val vATDueToHMRCRoute = routes.VATDueToHMRCController.onPageLoad(NormalMode).url

  "VATDueToHMRC Controller" must {

    "return OK and the correct view for a GET" in {

      val application = applicationBuilder(userAnswers = Some(emptyUserAnswers)).build()

      val request = FakeRequest(GET, vATDueToHMRCRoute)
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(VATDueToHMRCPage, "answer").success.value
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, vATDueToHMRCRoute)
