...
class WhomToPayControllerSpec extends SpecBase with MockitoSugar {
  ...
  "WhomToPay Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(WhomToPayPage, WhomToPay.values.head).success.value
      ...
      contentAsString(result) mustEqual
        view(form.fill(WhomToPay.values.head), NormalMode, backLink)(fakeRequest, messages).toString
