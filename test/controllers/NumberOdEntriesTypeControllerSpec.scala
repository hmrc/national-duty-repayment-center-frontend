...
import models.{NormalMode, NumberOfEntriesType, UserAnswers}
...
class NumberOfEntriesTypeControllerSpec extends SpecBase with MockitoSugar {
  ...
  "NumberOfEntriesType Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(NumberOfEntriesTypePage, NumberOfEntriesType.values.head).success.value
      ...
      contentAsString(result) mustEqual
        view(form.fill(NumberOfEntriesType.values.head), NormalMode)(fakeRequest, messages).toString
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, numberOfEntriesTypeRoute)
          .withFormUrlEncodedBody(("value", NumberOfEntriesType.options.head.value))
