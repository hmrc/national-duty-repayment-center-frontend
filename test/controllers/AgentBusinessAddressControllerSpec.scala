...
import forms.{AddressSelectionFormProvider, AgentBusinessAddressFormProvider, PostcodeFormProvider}
...
import pages.ImporterPostcodePage
...
import views.html.{AgentBusinessAddressConfirmationView, AgentBusinessAddressView}

import scala.concurrent.Future

class AgentBusinessAddressControllerSpec extends SpecBase with MockitoSugar {
  private lazy val agentBusinessAddressPostcodeRoute = routes.AgentBusinessAddressController.onPageLoad(NormalMode).url
  private lazy val agentBusinessAddressSelectRoute = routes.AgentBusinessAddressController.addressSelectSubmit.url
  private val postcodeForm = (new PostcodeFormProvider) ()
  private val addressForm = (new AgentBusinessAddressFormProvider) ()
  private val selectionForm = (new AddressSelectionFormProvider) ()

  def onwardRoute: Call = Call("GET", "/national-duty-repayment-center/enter-agent-business-address")
  ...
  "AgentBusinessAddressController" must {
    "return OK and the correct view for a GET on the postcode page" in {

      val application = applicationBuilder(userAnswers = Some(emptyUserAnswers)).build()
      running(application) {
        val request = buildRequest(GET, agentBusinessAddressPostcodeRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[AgentBusinessAddressView]
    ...
    "return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        ...
        val request = buildRequest(POST, agentBusinessAddressPostcodeRoute)
        ...
        val expectedView = application.injector.instanceOf[AgentBusinessAddressView]
    ...
    "redirect to the next page when an address has been selected" in {
      ...
      running(application) {
        val request = buildRequest(POST, agentBusinessAddressSelectRoute)
      ...
      "return a Bad Request and errors when nothing is selected" in {
        ...
        running(application) {
          ...
          val expectedView = application.injector.instanceOf[AgentBusinessAddressConfirmationView]
